"""
Telegram-бот v7 — исправлены переговоры, добавлены в /new меню.
"""
...
ВАЖНО: Если пользователь просит сохранить в файл/Word — делай это.
...
NEGOTIATION_QUESTIONS = [
    ...
    "Главная цель переговоров — снижение цены, проверка компетенций или другое?",
]

NEGOTIATION_SYSTEM = """Ты эксперт по переговорам в сфере тендеров и закупок.
Составляй профессиональные сценарии переговоров с участниками тендера.
Сценарий включает:
- Цель переговоров
- Подготовительный этап
- Ключевые вопросы к участнику
- Возможные возражения и ответы на них
- Критерии оценки участника по итогам
...
def main_menu_kb():
    """Главное меню после /new — включает все типы документов."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "📄 ТЗ, критерии допуска, сценарии переговоров → /new\n"
...
# ─── Переговоры (надёжная версия через ConversationHandler) ─────────────────
...
async def generate_negotiation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context_text = "\n".join(
        f"{NEGOTIATION_QUESTIONS[i]}: {answers.get(f'q{i}', '')}"
        for i in range(len(NEGOTIATION_QUESTIONS))
        if answers.get(f"q{i}")
    )
    ...
    try:
        ...
        # Показываем превью
        preview = content[:2500] + ("...\n_(показан фрагмент)_" if len(content) > 2500 else "")
        await update.message.reply_text(f"🤝 Сценарий переговоров:\n\n{preview}")
        ...
        await update.message.reply_text(
            "Сохранить в файл? Удачи на переговорах! 😄",
            reply_markup=kb
        )
        return REVIEWING
        ...
        await update.message.reply_text("Что-то пошло не так 😕 → /new")
        return ConversationHandler.END


async def cb_save_negotiation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сохраняет сценарий в Word."""
    from docx_generator import generate_tz_docx
    q = update.callback_query
    await q.answer()
    uid = update.effective_user.id
    ...
    try:
        ...
        with open(path, "rb") as f:
            await q.message.reply_document(
                ...
                caption="🤝 Сценарий переговоров готов! Удачи 😄"
            )
        os.remove(path)
        sessions.pop(uid, None)
...
async def chat_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сохранить в Word
    if any(w in tl for w in ["сохрани в файл", "сделай файл", "в ворд", "сохрани в ворд", "скачать", "сохрани это"]):
...
async def cb_save_to_word(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    # Передаём message для совместимости с save_last_to_word
            ...
            messages=[{
                ...
                "content": [
                    ...
                    {"type": "text", "text": (
                        ...
                        "3. Предложи поисковый запрос для доп. информации\n"
