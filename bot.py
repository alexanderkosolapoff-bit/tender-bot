...
# ─── Промпты ─────────────────────────────────────────────────────────────────
CHAT_SYSTEM = """Ты - Джарвис, умный и немного саркастичный помощник по тендерам. Как Джарвис из Железного человека: чёткий, профессиональный, с лёгкой иронией. Помогаешь всегда, но без лишней воды.
...
ANALYSIS_SYSTEM = """Ты опытный эксперт по государственным и корпоративным закупкам (44-ФЗ, 223-ФЗ).
Твоя задача — провести экспертизу проекта технического задания организатора тендера.
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(cb_analysis_actions,
                                         pattern="^(save_analysis|analysis_question|analysis_done)$"))
