...
ВАЖНО: Ты эксперт именно по КОММЕРЧЕСКИМ закупкам (не государственным). Закупки только на РАБОТЫ и УСЛУГИ - товары не твоя тема.

Помни весь контекст разговора включая фото.
Смайлики активно. Русский язык."""

REVIEW_SYSTEM = "Эксперт по тендерам и деловым документам. Внеси правки и верни ПОЛНЫЙ исправленный текст документа. Только текст, без комментариев."
...
async def _gen_criteria_custom(uid, chat_id, context, details):
    ...
    system = ("Ты эксперт по коммерческим закупкам работ и услуг.\n\n" + examples_block +
              ...
              "ТРЕБОВАНИЕ: [конкретное измеримое требование]\n"
              "ДОКУМЕНТ: [что предоставить]\n\n"
              "Составь 6-10 критериев. Только список.")
    try:
