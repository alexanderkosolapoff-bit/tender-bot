...
Пользователь надиктовал голосовое сообщение. Извлеки из него намерение и параметры.
...
Верни JSON строго в формате:
{
  ...
  "params": {
    "вопрос из анкеты": "ответ пользователя"
  },
  "original_task": "краткое описание задачи своими словами"
}

Примеры:
- "нужно ТЗ на уборку офиса 800 метров в Казани ежедневно" →
  intent=tz, direction=cleaning, params={площадь: "800 кв.м", адрес: "Казань", периодичность: "ежедневно"}
- "сделай критерии на IT аутсорсинг 50 пользователей" →
  intent=criteria, direction=it, params={что закупаем: "IT-аутсорсинг", кол-во мест: "50"}
- "подготовь переговоры по клинингу снизить цену на 15 процентов" →
  intent=negotiation, params={что закупаем: "Клининговые услуги", снижение: "На 10-20%"}
- "что такое НМЦ" → intent=chat

Если параметр не упомянут — не включай его в params.
...
# ─── Чат ─────────────────────────────────────────────────────────────────────
async def handle_voice_smart(update, context, transcribed_text: str):
    """
    Умная обработка голосового сообщения.
    Извлекает намерение и параметры из свободной речи.
    """
    uid = update.effective_user.id

    preview = transcribed_text[:100] + ("..." if len(transcribed_text) > 100 else "")
    await update.message.reply_text(f"Услышал: {preview}\nАнализирую задачу...")
    ...
    try:
        ...
        raw = re_mod.sub(r'```.*?```', '', raw, flags=re_mod.DOTALL).strip()
        raw = re_mod.sub(r'```', '', raw).strip()
        data = json_mod.loads(raw)
    except Exception as e:
        logger.error(f"Voice extract error: {e}")
        # Если не смогли распознать — обрабатываем как обычный текст
        return False

    intent = data.get("intent", "chat")
    direction = data.get("direction")
    params = data.get("params", {})
    original_task = data.get("original_task", transcribed_text)

    # Обычный чат — не голосовой режим
    if intent == "chat":
        return False

    # Запускаем нужный режим с предзаполненными данными
    sessions.pop(uid, None); last_doc.pop(uid, None); context.user_data.clear()

    if intent == "analysis":
        context.user_data["intent_analysis"] = True
        await update.message.reply_text(
            "Загрузи документ для анализа (Word, PDF или txt)."
        )
        return True
    ...
    if intent == "letter":
        context.user_data["letter_mode"] = "new"
        context.user_data["letter_original"] = ""
        context.user_data["waiting_doc_edit"] = False
        # Генерируем письмо сразу по описанию
        from docx_generator import generate_tz_docx
        await update.message.reply_text("Составляю письмо...")
        resp = claude.messages.create(
            model="claude-sonnet-4-5", max_tokens=2000, system=LETTER_SYSTEM,
            messages=[{"role": "user", "content": "Задание: " + original_task + "\n\nСоставь деловое письмо."}]
        )
        letter_content = resp.content[0].text
        remember(uid, letter_content)
        last_doc[uid] = {"content": letter_content, "type": "letter", "name": "Pismo"}
        await update.message.reply_text("В каком формате сохранить?", reply_markup=word_pdf_kb())
        return True

    if intent == "negotiation":
        # Заполняем ответы из голосового
        neg_answers = {}
        param_map = {
            0: ["что закупаем", "закупаем", "услуга", "работа"],
            1: ["кто придёт", "представитель", "директор", "менеджер"],
            2: ["нмц", "цена", "стоимость", "бюджет", "сумма"],
            3: ["снижение", "снизить", "скидка", "процент"],
            4: ["конкуренты", "альтернатива", "участники"],
            5: ["цели", "дополнительно", "кроме цены"],
        }
        for step_idx, keywords in param_map.items():
            for key, val in params.items():
                if any(kw in key.lower() for kw in keywords):
                    neg_answers[step_idx] = {
                        "q": NEGOTIATION_STEPS[step_idx]["q"],
                        "a": val
                    }
                    break
    ...
    if intent in ("tz", "criteria", "both"):
        ...
        if not direction:
            # Направление не определено — спрашиваем
            context.user_data["doc_type"] = doc_type
            await update.message.reply_text(
                "Понял задачу: " + original_task + "\nВыбери направление:",
                reply_markup=dir_kb()
            )
            return True

        # Создаём агента с предзаполненными данными
        ...
        for q_item in dir_questions:
            ...
            for param_key, param_val in params.items():
                if any(word in q_text for word in param_key.lower().split()):
                    agent.answers.append({"question": q_item["question"], "answer": param_val})
                    agent.current_q += 1
                    break

        filled = len(agent.answers)
        total = len(dir_questions)

        if filled > 0:
            filled_txt = "\n".join("- " + a["question"] + ": " + a["answer"] for a in agent.answers)
            await update.message.reply_text(
                "Понял! Из голосового заполнил " + str(filled) + " из " + str(total) + " параметров:\n" + filled_txt
            )
        ...
        if result["status"] == "question":
            ...
            await do_generate(FakeUpdate(), context)
