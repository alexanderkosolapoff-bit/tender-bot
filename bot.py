...
class TenderAgent:
    ...
    async def generate_tz(self) -> str:
        ...
        system = (
            ...
            f"ВАЖНО — используй ТАБЛИЦЫ там, где это уместно (как в примерах выше):\n"
            f"- перечень объектов/адресов;\n"
            f"- перечень работ с периодичностью;\n"
            f"- график оказания услуг;\n"
            f"- расценки и нормативы;\n"
            f"- время реагирования по уровням критичности;\n"
            f"- любые структурированные данные где есть колонки.\n\n"
            f"Таблицы оформляй строго в формате Markdown:\n"
            f"| Заголовок1 | Заголовок2 | Заголовок3 |\n"
            f"| --- | --- | --- |\n"
            f"| значение | значение | значение |\n\n"
            f"Сплошной текст оставляй для разделов: введение, термины, общие положения, "
            f"требования к исполнителю, порядок сдачи-приёмки и т.п.\n"
            f"Сам реши где таблица уместна, а где обычный текст — ориентируйся на примеры.\n\n"
            f"Заголовки разделов оформляй как '## Название раздела'.\n"
