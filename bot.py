...
CRITERIA_SYSTEM = """Эксперт по тендерам. Составь список критериев допуска.
Нумерованный список, каждый одной строкой. 5-10 критериев. Только список."""
...
async def cmd_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Отменено. Пиши если что!", reply_markup=main_kb())
    return ConversationHandler.END
...
async def cb_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = update.effective_user.id
    if q.data == "review_ok":
        await q.edit_message_text("Отлично, не зря старался! Для нового запроса нажми кнопку внизу.", reply_markup=main_kb())
